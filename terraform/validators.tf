...
data "template_file" "user_data" {
  ...
  vars = {
    ...
    network-peers = "s3://${aws_s3_bucket.config.id}/${aws_s3_bucket_object.network_peers.id}"
    consensus-peers = "s3://${aws_s3_bucket.config.id}/${aws_s3_bucket_object.consensus_peers.id}"
  }
}
...
data "local_file" "consensus_keys" {
  count    = length(var.peer_ids)
  filename = "${var.validator_set}/validator_${var.peer_ids[count.index]}.node.consensus.keys.toml"
}
...
data "local_file" "network_keys" {
  count    = length(var.peer_ids)
  filename = "${var.validator_set}/validator_${var.peer_ids[count.index]}.node.network.keys.toml"
}
