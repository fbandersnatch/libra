...
impl NodeConfig {
    ...
    pub fn load_config<P: AsRef<Path>>(mut peer_id: Option<String>, path: P) -> Result<Self> {
        ...
        for network in &mut config.networks {
            ...
            if network.role == "validator" {
                ...
                network.load_config(peer_id.take(), path.as_ref())?;
                validator_count += 1;
            } else {
                network.load_config(None, path.as_ref())?;
            }
        }
        config.consensus.load_config(path.as_ref())?;
